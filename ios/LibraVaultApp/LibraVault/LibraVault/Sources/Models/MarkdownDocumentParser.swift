...
extension MarkdownDocumentParser {
    static func chaptersForNarration(from blocks: [MarkdownBlock]) -> [BookChapter] {
        ...
        var currentSegments: [NarrationSegment] = []
        // A `.thematicBreak` has no text of its own (narrationText returns nil
        // for it), so its scene-break signal can't attach to a segment in the
        // same iteration — it's held here and applied to the *next* block that
        // actually produces a segment, restoring the pause #499's design
        // flagged as silently dropped before this existed.
        var pendingPause: NarrationSegment.PauseHint = .none

        func flush() {
            let text = currentParts.joined(separator: "\n\n")
            if !text.isEmpty {
                chapters.append(BookChapter(title: currentTitle ?? "Untitled", text: text, segments: currentSegments))
            }
            currentParts = []
            currentSegments = []
            pendingPause = .none
        ...
        for block in blocks {
            ...
            if !blockSegments.isEmpty {
                if pendingPause != .none {
                    ...
                    pendingPause = .none
                } else if !currentSegments.isEmpty, blockSegments[0].pauseBefore == .none {
                    // Every top-level block in this loop is separated by
                    // "\n\n" on the flat-text path (currentParts.joined) —
                    // a plain top-level paragraph with no pause hint of its
                    // own (headings/blockQuote already set .paragraph;
                    // list items 2+ already set .sentence) would otherwise
                    // run straight into whatever came before it with zero
                    // separation once flattened via plainText/SSML. Only
                    // upgrades an unset .none, and only when this isn't the
                    // very first segment of the chapter (no leading pause
                    // needed there).
                    blockSegments[0] = blockSegments[0].withPauseBefore(.paragraph)
                }
            }
            currentSegments.append(contentsOf: blockSegments)
        }
        flush()
        return chapters
    }
