...
final class DomainBridgeTests: XCTestCase {
    ...
    /// method's own doc comment. This is the "delete library items for this folder"
    /// half; a removed book's bookmarks, highlights, and reading progress must all go.
    func testRemoveFolderClearsBookmarksHighlightsAndProgressForGivenBookIds() async throws {
        ...
        try await bridge.removeFolder(bookIds: ["removed-1"])
    ...
    /// Regression guard: only the books that actually belonged to the removed folder
    ...
    func testRemoveFolderLeavesOtherBooksReadingDataUntouched() async throws {
    ...
    func testRemoveFolderThrowsWhenBridgeNotInitialized() async {
        ...
        do {
            try await uninitialized.removeFolder(bookIds: ["1"])
...
final class DomainBridgePersistenceTests: XCTestCase {
    ...
    /// Regression guard: `removeFolder`'s cleanup must actually persist, not just
    ...
    func testRemoveFolderCleanupPersistsAcrossBridgeInstances() async throws {
        let persistence = makeIsolatedPersistence()
        let first = LibravaultDomainBridge(persistence: persistence)
        try await first.initialize()
        ...
        try await first.removeFolder(bookIds: ["removed-1"])
