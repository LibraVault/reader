...
/**
 * Regression guard for issue #530 L5: the vault content screens used to read
 * [VaultScreenSecurityPreference.isEnabled] once via a keyless
 * `remember { }`, so toggling "Screen Security" in Settings only took effect
 * the next time the screen was recomposed from scratch (leave + re-enter),
 * contradicting [SecureScreenEffect]'s own doc comment ("toggling restores
 * normal behavior immediately"). [rememberScreenSecurityEnabled] fixes this
 * by observing the preference reactively.
 */
