...
class GoogleCloudTtsAdapterTest {
    ...
    fun `synthesize base64-decodes the JSON audioContent field, unlike the other four vendors' raw-bytes response`() = runTest {
        ...
        server.enqueue(MockResponse().setResponseCode(200).setBody("{\"audioContent\":\"$base64Audio\"}"))
        ...
        assertTrue(recorded.path!!.contains("key=goog-key"))
        assertTrue(recorded.body.readUtf8().contains("\"languageCode\":\"en-US\""), "expected locale derived from the voice id")
    }
    ...
    fun `synthesize tolerates unknown fields in the response JSON`() = runTest {
        ...
        server.enqueue(MockResponse().setResponseCode(200).setBody("{\"audioContent\":\"$base64Audio\",\"timepoints\":[]}"))
