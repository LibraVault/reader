...
class AzureSpeechAdapterTest {
    ...
    fun `synthesize sends SSML with the subscription key header and output format, returns raw audio bytes`() = runTest {
        ...
        speechServer.enqueue(MockResponse().setResponseCode(200).setBody(okio.Buffer().write(fakeAudio)))
        ...
        val ssmlBody = recorded.body.readUtf8()
        assertTrue(ssmlBody.contains("en-US-JennyNeural"))
        assertTrue(ssmlBody.contains("hello &amp; goodbye"), "text must be XML-escaped in the SSML body")
    }

    @Test
    fun `synthesize fails closed on a non-2xx response`() = runTest {
        speechServer.enqueue(MockResponse().setResponseCode(401))
    ...
    fun `a quote in voiceId is XML-escaped, not injected raw into the SSML attribute`() = runTest {
        speechServer.enqueue(MockResponse().setResponseCode(200).setBody(okio.Buffer().write(byteArrayOf(1))))
        adapter.synthesize("hi", "en-US-O'Brien'Neural", credentials)

        val recorded = speechServer.takeRequest()
        val ssml = recorded.body.readUtf8()
    ...
    fun `validateKey calls the token-issuance host, not the speech synthesis host`() = runTest {
        tokenServer.enqueue(MockResponse().setResponseCode(200).setBody("fake-token"))
        val result = adapter.validateKey(credentials)
        assertTrue(result.isSuccess)
        val recorded = tokenServer.takeRequest()
        assertEquals("azure-key", recorded.headers["Ocp-Apim-Subscription-Key"])
        assertTrue(recorded.path!!.contains("issueToken"))
    }
}
