...
class PlayBillingProGate(context: Context) : IProGate, PurchasesUpdatedListener {
    ...
    companion object {
        ...
        /**
         * Pure predicate — extracted to the companion so it's unit-testable on the
         * JVM without constructing a [PlayBillingProGate] instance (the constructor
         * calls the real `BillingClient.newBuilder(context)`, which isn't safe
         * outside a real Android runtime).
         */
        @JvmStatic
        internal fun isProPurchase(purchase: Purchase): Boolean =
