...
LibraVault iOS builds and uploads to TestFlight entirely through GitHub Actions — no local Mac build is required for a release. The library is real vault-scanned data only (`LibraryFileScanner`) — the hardcoded demo library that used to back a no-vault first launch has been removed. There is no real Kotlin domain-layer integration yet, but content itself is real and native to iOS: EPUB/PDF chapter text (`EPUBParser`/`PDFParser`/`BookContentProvider`), TTS (`AVSpeechSynthesizer`), and audiobook playback (`AVAudioPlayer`/`AudioPlaybackEngine`) all read/produce genuine content — `mobi`/`cbz` show an honest "not yet supported" state rather than any fake content.
...
- **No real KMP `core:domain` integration.** Content parsing/playback is real (native Swift — EPUBParser/PDFParser/AVSpeechSynthesizer/AVAudioPlayer), just not backed by the originally-planned Kotlin domain layer — see "Relationship to the Phase D KMP docs" below. `mobi`/`cbz` aren't parsed (Android doesn't parse them either); audiobook chapter markers aren't extracted from m4b metadata yet (skip-forward/backward does real seeking regardless).
...
- May still be a real future goal, but should be scoped and estimated fresh against the current codebase rather than trusting those docs' effort estimates, which were written before this session's discovery that even the basic "does an app target exist" question was unresolved. Real content parsing/playback (EPUB/PDF/TTS/audiobooks) was built natively in Swift instead — no mock content or hardcoded demo library remains anywhere in the app — so this KMP path was never on the critical path for that work.
