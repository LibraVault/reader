...
| Encrypted Vault manifest (titles, authors, bookmarks, highlights, cover art) | Same dir as content, encrypted as one manifest file — never Room, never the plaintext cover-art cache | `core/vaultstore/src/test/.../VaultLeakClosureTest.kt`, `.../VaultStoreHasNoLeakSurfaceDependencyTest.kt` |
...
| Vault content visible via screenshot/screen recording/casting | User (or malware) captures the screen while a vault is unlocked | 3 | 3 | **Android**: `FLAG_SECURE` while vault content is on screen — a real, OS-enforced *block* on screenshots, screen recording, and non-secure mirroring. Unconditional on the recovery-key display/entry steps; user-togglable (default on) elsewhere via the "Screen Security" setting. **iOS**: no equivalent API exists to block a capture outright (`ScreenCaptureMonitor`'s own doc comment covers why — no documented way to make a still screenshot fail, and blocking a screen *recording*/AirPlay mirror pixel-for-pixel while leaving the live screen untouched needs undocumented view-hierarchy tricks, not a public flag). The achievable ceiling is **detect-and-react**, weaker than Android's hard block by design, not by omission: `ScreenCaptureMonitor` reports `UIScreen.isCaptured` (recording/mirroring) and blanks content for *everyone* (recorder and live viewer alike) while it's true, and separately reports `userDidTakeScreenshotNotification` (fires only *after* the image is already saved to Photos — nothing can preempt it). `VaultContentSecurityModifier` reuses `ScreenCaptureMonitor` for general vault content (unconditional use is `SecureScreenModifier`, recovery-key screens only) and adds a second, independent blank-and-react path for the app-switcher snapshot (`willResignActiveNotification`, which fires *before* `didEnterBackground` takes the snapshot) plus an auto-lock reaction on capture-start, user-togglable (default on) via `VaultScreenSecurityPreference`, matching Android's setting name/default/scope. A screenshot on iOS is still never blocked or auto-locked-against — only detected after the fact on the recovery-key screens that already warn about it. | Android: `feature/vault/.../SecureScreenEffect.kt`, `feature/vault/src/test/.../VaultScreenSecurityPreferenceTest.kt`. iOS: `Sources/Features/EncryptedVault/{ScreenCaptureMonitor,SecureScreenModifier,VaultContentSecurityModifier,VaultContentSecurityState,VaultScreenSecurityPreference}.swift`, `LibraVaultTests/{ScreenCaptureMonitorTests,VaultContentSecurityStateTests,VaultScreenSecurityPreferenceTests}.swift` |
...
| Vault content leaks into the app's existing unencrypted surfaces | Title/author/cover art/bookmarks/highlights accidentally routed to Room or the plaintext cover-art cache | 2 | 5 | `VaultStore` has no code path to Room or `core.storage.CoverArtCache` — asserted by reflection, not just "we didn't call it"; separately, `MetadataExtractor.extractWithoutCaching` is the only cover-art path the vault import flow uses, and it never calls `CoverArtCache.save`; decrypted cover thumbnails shown in `VaultContentsScreen` are decoded to a `Bitmap` only inside Compose, never written to disk | `core/vaultstore/src/test/.../VaultStoreHasNoLeakSurfaceDependencyTest.kt`, `.../VaultLeakClosureTest.kt`, `core/storage/src/test/.../MetadataExtractorTest.kt` |
| Cloud TTS sends reader text to a third-party vendor | Premium Cloud TTS Voices (BYOK) is enabled and a chapter is read aloud | 2 | 3 | Two independent switches both required before any network call — real `subscriptionActive`/`isSubscribed` (not a flag) AND a separate, off-by-default consent toggle with its own disclosure; enforced at the lowest seam (`CloudTtsEngine`/`CloudTtsGate`, re-checked before every chunk, not just once) so no other call path can bypass it. Five vendor hosts fixed and pinned, no arbitrary/custom endpoint. BYOK: the vendor API key is Keystore/Keychain-wrapped, never logged, LibraVault never sees or proxies it. Falls back to on-device TTS on any cloud error rather than stalling. The Settings-side key-validation call (`CloudVoicesSection.validateAndSave`/Android's `onValidateAndSaveCloudKey`) is the one other call site that reaches a vendor host; it's gated by the same "Settings section only rendered when subscribed" + "provider list only shown once consent is on" chain, not a separate bypass. | `core/cloudtts/src/main/.../CloudTtsGate.kt`, `.../CloudTtsEngine.kt`, `.../RealCloudApiKeyStore.kt`, `core/cloudtts/src/testPlay/.../CloudTtsFixedHostsRegressionTest.kt`, `core/cloudtts/src/test/.../CloudTtsGateTest.kt`. iOS: `Sources/Features/CloudTTS/{CloudTtsGate,KeychainCloudApiKeyStore,CloudVoicesSection,CloudTtsEngine}.swift`, `LibraVaultTests/CloudTTS/{CloudTtsGateTests,CloudTtsFixedHostsRegressionTests,CloudVoicesSectionTests,CloudTtsEngineTests}.swift` — iOS's own `CloudTtsEngine` (the playback-time enforcement point, `TTSEngineType.cloud`) merged via #478; the Settings UI now also captures the per-provider Voice ID `CloudTtsEngine.performSpeak` requires, so cloud playback is actually reachable end-to-end. |
...
- Desktop ports — separate workstream, not started. iOS Encrypted Vaults (create/unlock/list, import, Screen Security) has landed — see `docs/ios-android-feature-parity.md`'s Encrypted Vaults row for current status; per-format read/play integration (EPUB/PDF/audio) is still in progress.
