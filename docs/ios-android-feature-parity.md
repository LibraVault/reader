...
| Encrypted Vaults | ✅ full: create/unlock/list, import, read/play (EPUB/PDF/audio), cover-art thumbnails, bookmarks (EPUB+PDF)/highlights (EPUB), per-user reading settings (EPUB), Screen Security toggle | ✅ create/unlock/list ([#334](https://github.com/LibraVault/reader/issues/334)), import ([#345](https://github.com/LibraVault/reader/issues/345)), read/play (EPUB/PDF/audio) via `VaultReaderView`/`VaultPlayerView` ([#203](https://github.com/LibraVault/reader/issues/203)), bookmarks (EPUB+PDF+audio)/highlights (EPUB+PDF, whole-chapter/page granularity — no text-selection UI yet on either vault or non-vault iOS reading) | iOS gaps still open: no cover-art thumbnails in the vault contents list, no per-user reading-settings persistence for vault content (deliberately — matches Android's own session-only behavior there, see #203), no Screen Security *toggle* (vault reader/player screens unconditionally blank on screen recording via `.secureVaultScreen()`, narrower than Android's toggle), no audio streaming (`AVAudioPlayer(data:)` decrypts the whole file into memory, matching Android's own fallback tier). Not yet on Android either: vault audio bookmarks (#177), a paginated non-scrolling vault PDF mode (#178) |
