...
| Licensing / Pro unlock | ⚠️ `LicenseVerifier.PUBLIC_KEY_B64` is still a placeholder — F-Droid Pro unlock is non-functional | ❌ not wired at all (`AppState.isSupporter` is hardcoded `false`) | Same underlying gap (no real license backend yet), further along on Android |
